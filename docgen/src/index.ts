 ...
 * Files with rootRelative: true are written relative to rootDir instead of outputDir.
 * A provenance header is prepended to all .md files.
 */
function writeRenderedFiles(
  files: { path: string; content: string; rootRelative?: boolean }[],
  outputDir: string,
  rootDir?: string,
): void {
  const provenanceHeader = "<!-- Generated by Generic-doc-generator. Do not edit manually. Regenerate with: bash generate-all.sh --generate-all --target <project-root> -->\n";
  for (const file of files) {
    const baseDir = file.rootRelative && rootDir ? rootDir : outputDir;
    const fullPath = join(baseDir, file.path);
    mkdirSync(dirname(fullPath), { recursive: true });
    let content = file.content;
    if (file.path.endsWith(".md")) {
      content = provenanceHeader + content;
    }
  ...
  try {
    ...
    if (args.command === "generate") {
      ...
      if (!args.jsonOnly) {
        ...
        writeRenderedFiles(rendered, fullOutputDir, rootDir);
